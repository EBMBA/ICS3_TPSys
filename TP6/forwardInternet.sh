...
( sudo iptables -t nat -a POSTROUTING -s 192.168.100.0/24 -o $VARINTERFACE1 -j MASQUERADE && sudo iptables-persistent) && echo "Masquerading enabled" || echo "Masquerading disabled"